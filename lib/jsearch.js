...
// Google-for-Jobs style natural-language search parses best as simple
// "<title> jobs" / "<title> jobs in <place>" phrases — a single compound
// query like "... remote or hybrid in Toronto ..." was returning zero
// matches, so run one broad pass (catches remote postings) and one
// Toronto-anchored pass (catches GTA postings) and let our own
// isRemote/isGta classification do the real filtering afterward.
async function fetchTitleFromApi(title, apiKey) {
  const [general, toronto] = await Promise.all([
    runQuery(`${title} jobs`, apiKey),
    runQuery(`${title} jobs in Toronto, Ontario, Canada`, apiKey),
  ]);
  // Tag each raw result with which query pass found it, so we can show a
  // real breakdown of what each of the 8 daily queries actually contributes.
  general.forEach((item) => { item.__matchedQuery = "general"; });
  toronto.forEach((item) => { item.__matchedQuery = "toronto"; });
  return [...general, ...toronto];
}
...
async function fetchAllJobs(apiKey) {
  ...
  for (const title of JOB_TITLES) {
    try {
      ...
      rawCounts[title] = {
        general: { raw: 0, qualified: 0 },
        toronto: { raw: 0, qualified: 0 },
      };
      for (const item of raw) {
        const bucket = rawCounts[title][item.__matchedQuery];
        if (bucket) bucket.raw += 1;
        const job = normalizeJob(item, title);
        if (!job) continue;
        if (bucket) bucket.qualified += 1;
        // Keep the highest-scoring match if the same job matched multiple
        // titles/queries, but remember every query pass that surfaced it —
        // that's what lets us report which of the 8 daily queries actually
        // contributed each result.
        const existing = byId.get(job.id);
        if (!existing) {
          byId.set(job.id, job);
        } else {
          const merged = job.score > existing.score ? job : existing;
          const other = merged === job ? existing : job;
          merged.matchedQuery = Array.from(
            new Set([...merged.matchedQuery.split("+"), ...other.matchedQuery.split("+")])
          ).join("+");
